...
class DataMem(object):
    ...
    def output_data_mem(self, output_path=None):
        if output_path is None:
            output_path = self.io_dir  # Fall back to the original io_dir if no path is provided
        if self.id == 'SS':
            res_path = os.path.join(output_path, self.id + "_DMEMResult.txt")
        else:
            res_path = os.path.join(output_path, self.id + "_DMEMResult.txt")
